...
/*   Updated: 2025/04/30 15:52:20 by mohaben-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

int	parsing_error(char *err)
{
	printf("%s\n", err);
	return (0);
}
...
{
	if (ac < 5 || ac > 6)
		return (parsing_error("Invalid number of arguments!"));
	data->philos_nb = ft_atoi(av[1]);
	if (data->philos_nb <= 0 || data->philos_nb > 200)
		return (parsing_error("Invalid number of philosophers!"));
	data->die_time = ft_atoi(av[2]);
	data->eat_time = ft_atoi(av[3]);
	data->sleep_time = ft_atoi(av[4]);
	if (data->die_time < 60 || data->eat_time < 60 || data->sleep_time < 60)
		return (parsing_error("Time value should not be lower than 60 ms!"));
	if (ac == 6)
	{
		data->meals_required = ft_atoi(av[5]);
		if (data->meals_required < 0)
			return (parsing_error("Invalid number of times each philosopher must eat!"));
	}
	else
	...
	if (!create_data_mutexs(data))
		return (parsing_error("Failed to create Mutex!"));
	return (1);
}
